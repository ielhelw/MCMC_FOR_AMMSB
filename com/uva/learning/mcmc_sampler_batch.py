...
class MCMCSamplerBatch(Learner):
    ...
    def __init__(self, args, graph):
        ...
        print "Use fixed values (1.0, 100.0) for eta i.s.o. command-line params"
        self.__theta = random.gamma(1,100,(self._K, 2))      # parameterization for \beta
        # print "Use fixed values (100.0, 1.0) for eta i.s.o. command-line params"
        # self.__theta = random.gamma(100,1,(self._K, 2))      # parameterization for \beta
    ...
    def __update_beta(self):
        ...
        for i in range(0, self._N):
            for j in range(i+1, self._N):
                ...
                for k in range(0, self._K):
                    grads[k,0] += (probs[k]/sum_prob) * (abs(1-y)/self.__theta[k,0]-1/sum_theta[k])
                    grads[k,1] += (probs[k]/sum_prob) * (abs(-y)/self.__theta[k,1]-1/sum_theta[k])
    ...
    def update_phi(self,i):
        ...
        for j in range(0, self._N):
            ...
            for k in range(0, self._K):
                grads[k] += (probs[k]/sum_prob)/self.__phi[i][k] - 1.0/sum_phi
    ...
    def run(self):
        ...
        print "Don't override command-line parameter max_iteration to fixed value 300"
