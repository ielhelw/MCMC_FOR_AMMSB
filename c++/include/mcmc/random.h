...
class Random {
 ...
 public:
...
#endif
    srand(seed);
    std::cerr << "Random seed [" << xorshift_state[0] << ","
              << xorshift_state[1] << "]" << std::endl;
  }
  ...
  uint64_t seed(int x) const { return xorshift_state[x]; }
  ...
  const uint64_t seed(int x) const { return 0; }
...
class FileReaderRandom : public Random {
 ...
 public:
  ...
  List *sample(const List &population, ::size_t count) {
    ...
    // std::cerr << "Read " << count << " random.sample<List> values" <<
    // std::endl;
    return result;
  }
  ...
  std::vector<Element> *sample(const std::vector<Element> &population,
    ...
    getline(sampleReader, line);
    std::istringstream is(line);
    // // std::cerr << "Read vector<something>[" << count << "] sample; input
    // line '" << is.str() << "'" << std::endl;
    ...
    // std::cerr << "Read " << count << " random.sample<vector> values" <<
    // std::endl;
