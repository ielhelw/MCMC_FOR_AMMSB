...
static const uint64_t ktab[128] = {
    0,        12590644, 14272653, 14988939, 15384584, 15635009, 15807561,
    15933577, 16029594, 16105155, 16166147, 16216399, 16258508, 16294295,
    16325078, 16351831, 16375291, 16396026, 16414479, 16431002, 16445880,
    16459343, 16471578, 16482744, 16492970, 16502368, 16511031, 16519039,
    16526459, 16533352, 16539769, 16545755, 16551348, 16556584, 16561493,
    16566101, 16570433, 16574511, 16578353, 16581977, 16585398, 16588629,
    16591685, 16594575, 16597311, 16599901, 16602354, 16604679, 16606881,
    16608968, 16610945, 16612818, 16614592, 16616272, 16617861, 16619363,
    16620782, 16622121, 16623383, 16624570, 16625685, 16626730, 16627708,
    16628619, 16629465, 16630248, 16630969, 16631628, 16632228, 16632768,
    16633248, 16633671, 16634034, 16634340, 16634586, 16634774, 16634903,
    16634972, 16634980, 16634926, 16634810, 16634628, 16634381, 16634066,
    16633680, 16633222, 16632688, 16632075, 16631380, 16630598, 16629726,
    16628757, 16627686, 16626507, 16625212, 16623794, 16622243, 16620548,
    16618698, 16616679, 16614476, 16612071, 16609444, 16606571, 16603425,
    16599973, 16596178, 16591995, 16587369, 16582237, 16576520, 16570120,
    16562917, 16554758, 16545450, 16534739, 16522287, 16507638, 16490152,
    16468907, 16442518, 16408804, 16364095, 16301683, 16207738, 16047994,
    15704248, 15472926};
...
uint64_t Random::seed(int x) const { return xorshift_state[x]; }
...
std::unordered_set<int> Random::sample(int from, int upto, ::size_t count) {
...
double Random::gsl_ran_gaussian_ziggurat(const double sigma) {
  ...
  while (1) {
    TICK(iters_randn);
    if (range >= 0xFFFFFFFF) {
      uint64_t k = rand() - offset;
      i = (k & 0xFF);
      j = (k >> 8) & 0xFFFFFF;
    } else if (range >= 0x00FFFFFF) {
      uint64_t k1 = rand() - offset;
      uint64_t k2 = rand() - offset;
      i = (k1 & 0xFF);
      j = (k2 & 0x00FFFFFF);
    } else {
      i = randint(0, 256 - 1);      /*  choose the step */
      j = randint(0, 16777216 - 1); /* sample from 2^24 */
    }
...
void Random::report() {
  ...
  if (n_randn > 0) {
    std::cerr << "randn calls " << n_randn << " iters "
              ...
              << (1.0 * ktab_exceed_randn / n_randn) << " log/exp calls "
              << (1.0 * log_exp_randn / n_randn) << std::endl;
  }
#endif
}
