...
#define gsl_rng_get(randomState)				rand(randomState)
#define gsl_rng_uniform(randomState)			random(randomState)
...
gsl_ran_gaussian_ziggurat (gsl_rng *randomState, const double sigma)
{
  ...
  const ulong range = 0xffffffffUL;
  ...
  while (1)
    {
      if (range >= 0xFFFFFFFF)
        {
          ulong k = gsl_rng_get(randomState) - offset;
          i = (k & 0xFF);
          j = (k >> 8) & 0xFFFFFF;
        }
      else if (range >= 0x00FFFFFF)
        {
          ulong k1 = gsl_rng_get(randomState) - offset;
          ulong k2 = gsl_rng_get(randomState) - offset;
          i = (k1 & 0xFF);
          j = (k2 & 0x00FFFFFF);
        }
      else
        {
          i = gsl_rng_uniform_int (randomState, 256); /*  choose the step */
          j = gsl_rng_uniform_int (randomState, 16777216);  /* sample from 2^24 */
        }
      ...
      if (i < 127)
        {
          ...
          y0 = gsl_ytab[i];
          y1 = gsl_ytab[i + 1];
          U1 = gsl_rng_uniform (randomState);
          y = y1 + (y0 - y1) * U1;
        }
      else
        {
          double U1, U2;
          U1 = 1.0 - gsl_rng_uniform (randomState);
          U2 = gsl_rng_uniform (randomState);
